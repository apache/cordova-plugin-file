...
(function () {
    ...
    if (window.cordovaFilePlugin === undefined) {
        console.error('Something went wrong while installing the plugin. Please remove the plugin and install it again.');
        return;
    }

    const nodePath = window.cordovaFilePlugin.path;
    ...
    (function (exports, global) {
        const pathsPrefix = window.cordovaFilePlugin.appPaths;
        ...
        exports.readEntries = function (successCallback, errorCallback, args) {
            ...
            window.cordovaFilePlugin.readFile(fullPath, (err, files) => {
                if (err) {
                    if (errorCallback) {
        ...
        exports.getFile = function (successCallback, errorCallback, args) {
            ...
            window.cordovaFilePlugin.stat(path, (err, stats) => {
                ...
                const exists = !err;
                const baseName = window.cordovaFilePlugin.path.basename(path);

                function createFile () {
                    window.cordovaFilePlugin.open(path, 'w', (err, fd) => {
                        if (err) {
                            if (errorCallback) {
                                errorCallback(FileError.INVALID_STATE_ERR);
                            }
                        ...
                        window.cordovaFilePlugin.close(fd, (err) => {
                            if (err) {
                                if (errorCallback) {
                                    errorCallback(FileError.INVALID_STATE_ERR);
                                }
        ...
        exports.getFileMetadata = function (successCallback, errorCallback, args) {
            const fullPath = args[0];
            window.cordovaFilePlugin.stat(fullPath, (err, stats) => {
                if (err) {
                    if (errorCallback) {
                        errorCallback(FileError.NOT_FOUND_ERR);
                    }
                ...
                const baseName = window.cordovaFilePlugin.path.basename(fullPath);
        ...
        exports.getMetadata = function (successCallback, errorCallback, args) {
            window.cordovaFilePlugin.stat(args[0], (err, stats) => {
                if (err) {
                    if (errorCallback) {
                        errorCallback(FileError.NOT_FOUND_ERR);
                    }
        ...
        exports.setMetadata = function (successCallback, errorCallback, args) {
            ...
            window.cordovaFilePlugin.utimes(fullPath, metadataObject.modificationTime, metadataObject.modificationTime, (err) => {
                if (err) {
                    if (errorCallback) {
                        errorCallback(FileError.NOT_FOUND_ERR);
                    }
        ...
        exports.write = function (successCallback, errorCallback, args) {
            ...
            const promisify = window.cordovaFilePlugin.util.promisify;
            let bytesWritten = 0;
            promisify(window.cordovaFilePlugin.open)(fileName, 'a')
                .then(fd => {
                    return promisify(window.cordovaFilePlugin.write)(fd, data, position)
                              .then(bw => { bytesWritten = bw; })
                              .catch(err => console.log(err))
                              .finally(() => promisify(window.cordovaFilePlugin.close)(fd));
                })
        ...
        exports.remove = function (successCallback, errorCallback, args) {
            ...
            window.cordovaFilePlugin.stat(fullPath, (err, stats) => {
                ...
                window.cordovaFilePlugin.rm(fullPath, (err) => {
                    if (err) {
                        if (errorCallback) {
        ...
        exports.truncate = function (successCallback, errorCallback, args) {
            ...
            window.cordovaFilePlugin.truncate(fullPath, size, err => {
                if (err) {
                    if (errorCallback) {
                        errorCallback(FileError.INVALID_STATE_ERR);
                    }
        ...
        exports.getDirectory = function (successCallback, errorCallback, args) {
            ...
            window.cordovaFilePlugin.stat(path, (err, stats) => {
                ...
                if (options.create === true && options.exclusive === true && exists) {
                    ...
                    // MyDirectoryEntry.
                    window.cordovaFilePlugin.mkdir(path, (err) => {
                        if (err) {
                            if (errorCallback) {
                                errorCallback(FileError.PATH_EXISTS_ERR);
                            }
        ...
        exports.copyTo = function (successCallback, errorCallback, args) {
            ...
            window.cordovaFilePlugin.copyFile(srcPath, dstDir + dstName, (err) => {
                if (err) {
                    if (errorCallback) {
                        errorCallback(FileError.INVALID_MODIFICATION_ERR);
                    }
    ...
    /** * Helpers ***/
        ...
        function readAs (what, fullPath, encoding, startPos, endPos, successCallback, errorCallback) {
            ...
            window.cordovaFilePlugin.open(fullPath, 'r', (err, fd) => {
                if (err) {
                    if (errorCallback) {
                        errorCallback(FileError.NOT_FOUND_ERR);
                    }
                ...
                const buf = window.cordovaFilePlugin.bufferAlloc(endPos - startPos);
                promisify(window.cordovaFilePlugin.read)(fd, buf, startPos)
                    .then(() => {
                        switch (what) {
                        case 'text':
